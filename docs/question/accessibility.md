...
原因1：某些情况下，当前A页面可能存在多个id相同的头像节点，你的代码拿到的可能不是在当前界面上的节点，导致点击了其他节点

> 解决方案：增加判断，确保你要获取的id在当前界面，可以通过位置信息确保节点在大概位置（或者判断是否可见等）


原因2：某些情况下，点击头像节点失效了，但是系统确实点击了（这种情况常见于某些APP框架，DeekeScript中暂未发现）

> 解决方案：在点击的时候，尽可能使用节点的特性，假设节点的clickable为true，则直接使用obj.click()即可；或者发现它的父节点的clickable为true，则直接使用obj.parent().click()；只有在clickable为false的时候，才去使用系统提供的位置点击方法。

原因3：某些情况下被弹窗遮挡了，位置点击的时候跑到其他界面去了

> 解决方案：对APP的弹窗进行全局的监控，发现之后进行关闭。

原因4：进入A页面的时候，没有设置合理的等待时间，导致头像节点还未加载成功，导致点击失败
...
> 解决方案：对列表执行完成进行判断（DeekeScript中，可以获取对象的_addr属性，比如一次获取到了3个节点，把_addr进行保存，然后滑动之后，再进行读取，与前面的比对，如果相同则认为滑动完成）
