...
1. **权限要求**：使用 MediaStore 功能前，需要先申请媒体权限
2. **版本兼容**：所有方法都自动处理不同 Android 版本的差异，无需手动判断版本
3. **返回值**：失败时返回空字符串 "" 或空数组 []，而不是 null
4. **异步操作**：权限申请是异步的，不会阻塞代码执行
5. **URI格式**：Android 10+ 返回 content:// URI，Android 9- 返回 file:// URI
